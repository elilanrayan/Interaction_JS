...
form.addEventListener('submit',function(event){
    ...
    if(liste.value==""){
        errorContainer.classList.add("visible")
        liste.classList.remove("green")

        let err=document.createElement('li')
        err.innerText="Merci de sélectionner votre région"
        errorList.appendChild(err)

    }else{
        liste.classList.add("green")
    }

    let successContainer=document.querySelector(".message-succes")
    successContainer.classList.remove("visible")

    if(pseudo.classList.contains("green")&&email.classList.contains("green")&&password.classList.contains("green")&&passwordRepeat.classList.contains("green")&&liste.classList.contains("green")){
